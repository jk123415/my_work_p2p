...
'''
class Gx1654QianPengWangPipeline(object):
...
class Gx1654QianPengWangPipeline(object):
    ...
    def process_item(self, item, spider):
        # period = re.subn('[\r\n\s]', "", item['period'])[0]
        period = re.search('元 (.*?)$', item['period']).group(1)
        period = re.subn('[\s]', "", period)[0]
        item['period'] = period
        ...
        # rate = re.subn('[\r\n\s]', "", item['rate'])[0]
        rate = re.search('^(.*?)\s', item['rate']).group(1)
        item['rate'] = rate

        # amount = re.subn('[\r\n\s]', "", item['amount'])[0]
        amount = re.search('\s(.*?)\s', item['amount']).group(1)
        item['amount'] = amount
