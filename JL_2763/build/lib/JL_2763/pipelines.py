...
class Jl2763Pipeline(object):
    ...
    def process_item(self, item, spider):
        period = re.search('\s(.*?)天\s', item['period'])
        item['period'] = str(eval(period.group(1))) + '天'

        # progress = re.subn('[\r\n\s]', "", item['progress'])
        # item['progress'] = progress[0]
        if item['progress'] == '<img src="/mdw/images/repayment_r.png">':
            item['progress'] = True

        rate = re.search('\s.*?\s(.*?)%', item['rate'])
        item['rate'] = rate.group(1)
        # rate = re.subn('[\r\n\t\s%]', "", item['rate'])
        # item['rate'] = str(eval(rate[0]))

        amount = re.search('￥(.*?)$', item['amount']).group(1)
        item['amount'] = amount
