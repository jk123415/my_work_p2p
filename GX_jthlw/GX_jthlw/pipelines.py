...
class GxJthlwPipeline(object):
    ...
    def process_item(self, item, spider):
        start = re.search('^(\d{4})(\d{2})(\d{2})\d+$',item['start'])
        item['start'] = start.group(1)+"-"+start.group(2)+"-"+start.group(3)
        end = re.search('^(\d{4})(\d{2})(\d{2})\d+$',item['end'])
        item['end'] = end.group(1)+"-"+end.group(2)+"-"+end.group(3)

        pay_type = item['pay_type']
        if re.search('按季付息，到期还本', pay_type):
            item['pay_type'] = '6'
        elif re.search('按月付息，到期还本', pay_type):
            item['pay_type'] = '4'
        elif re.search('一次性', pay_type):
            item['pay_type'] = '3'
        else:
            item['pay_type'] = '0'
...
class Publish34(object):
    def process_item(self, item, spider):
        ...
        conti = spider.collection.find_one({'item_code': item_code})
        if conti:
            spider.logger.info(item['title'] + '已经采集过')
            raise DropItem
